...
SCAN_URL = "https://confluxscan.io/address/cfx:aapwjebcay7d6jv02whjrrvkm9egmw5fye09cea6zz?NFTAddress=cfx%3Aacft8mk0tc2y7ngt8ud0h0cwzt74g8zauasfdmbf03&limit=50&skip={}&tab=nft-asset"
...
def return_fig_count(name):
    if name.startswith("衣服"):
        return 264
    elif name.startswith("首饰"):
        return 222
...
if __name__ == "__main__":
    ...
    fig_cnt_dict = {}
    for (_, name) in idrange.ShangShi_IdRange2Name.items():
        fig_cnt_dict[name] = 0
    for skip_count in range(0, contract_item_count-NEGLECT_COUNT, 50):
        ...
        for nft in nfts:
            ...
            if flag_pos != -1:
                ...
                for (range, name) in idrange.ShangShi_IdRange2Name.items():
    ...
    # dump file
    result_file = open("data/_scan_conflux_shangshi_result_{}.csv".format(
