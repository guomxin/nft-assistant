...
SCAN_URL = "https://confluxscan.io/address/cfx:aapwjebcay7d6jv02whjrrvkm9egmw5fye09cea6zz?NFTAddress=cfx%3Aacby6g5sg4v7cufsbrhbwccu591tpv30865ubenucn&limit=50&skip={}&tab=nft-asset"
...
if __name__ == "__main__":
    ...
    fig_cnt_dict = {}
    for (_, name) in idrange.ZhongGuoHangTian_IdRange2Name.items():
        fig_cnt_dict[name] = 0
    for skip_count in range(0, contract_item_count-NEGLECT_COUNT, 50):
        ...
        for nft in nfts:
            ...
            if token_id != None:
                for (range, name) in idrange.ZhongGuoHangTian_IdRange2Name.items():
    ...
    # dump file
    result_file = open("data/_scan_conflux_zhongguohangtian_result_{}.csv".format(
        datetime.strftime(datetime.now(), '%Y%m%d%H%M')
    ), "w")
    for (fig, cnt) in fig_cnt_dict.items():
        result_file.write("{},{}\n".format(fig, return_fig_count(idrange.ZhongGuoHangTian_IdRange2Name, fig) - cnt))
    result_file.close()
    driver.close()