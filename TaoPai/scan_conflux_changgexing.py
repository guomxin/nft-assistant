...
SCAN_URL = "https://confluxscan.io/address/cfx:aapwjebcay7d6jv02whjrrvkm9egmw5fye09cea6zz?NFTAddress=cfx%3Aach5xttx9vgxuu18hg2hhegpavrv6c7d3urdj6zbu8&limit=50&skip={}&tab=nft-asset"
...
if __name__ == "__main__":
    ...
    fig_cnt_dict = {}
    for (_, name) in idrange.ChangGeXing_IdRange2Name.items():
        fig_cnt_dict[name] = 0
    reach_min_id = False
    for skip_count in range(0, contract_item_count-NEGLECT_COUNT, 50):
        ...
        for nft in nfts:
            ...
            if token_id < MIN_TOKEN_ID:
                ...
                break
            if token_id != None:
                for (range, name) in idrange.ChangGeXing_IdRange2Name.items():
    ...
    # dump file
    result_file = open("data/_scan_conflux_changgexing_result_{}.csv".format(
        datetime.strftime(datetime.now(), '%Y%m%d%H%M')
    ), "w", encoding="utf-8-sig")
    for (fig, cnt) in fig_cnt_dict.items():
        result_file.write("{},{}\n".format(fig, utils.return_fig_count(idrange.ChangGeXing_IdRange2Name, fig) - cnt))
    result_file.close()
    driver.close()